...
ASSERTION_PARTITIONS={'facts.jsonl':'facts','contested.jsonl':'contested','unresolved.jsonl':'unresolved'}
DERIVED_SCHEMA_VERSION='0.2.0'
...
def create_schema(con):
 ...
 CREATE TABLE works(work_id TEXT PRIMARY KEY,record_json TEXT NOT NULL);
 CREATE TABLE evidence(evidence_id TEXT PRIMARY KEY,record_json TEXT NOT NULL);
 CREATE INDEX idx_assertions_status ON assertions(projection_status); CREATE INDEX idx_assertions_subject ON assertions(subject_type,subject); CREATE INDEX idx_assertions_predicate ON assertions(predicate);
 CREATE INDEX idx_provenance_assertion ON provenance(assertion_id); CREATE INDEX idx_reconciliation_subject ON reconciliation_history(subject_type,subject_id); CREATE INDEX idx_relations_subject ON relations(subject_type,subject_id); CREATE INDEX idx_relations_target ON relations(target_type,target_id);
 ''')
...
def build_database(projection_root,database_path):
 projection_root=Path(projection_root); database_path=Path(database_path); verified=bundle.verify_projection(projection_root); rows=verified['rows']; manifest=verified['manifest']
 database_path.parent.mkdir(parents=True,exist_ok=True); fd,tmp_name=tempfile.mkstemp(prefix=database_path.name+'.',suffix='.tmp',dir=database_path.parent); os.close(fd); os.unlink(tmp_name); tmp_path=Path(tmp_name)
 con=None
 try:
  con=sqlite3.connect(tmp_path); create_schema(con)
  builder_identity=bundle.tool_identity(Path(__file__),_MODULE)
  metadata={
   'projection_hash':manifest['projection_hash'],'input_hash':manifest['input_hash'],'projection_version':manifest['projection_version'],'schema_version':manifest['schema_version'],'methodology_version':manifest['methodology_version'],'research_head':manifest['research_head'],'reconciliation_head':manifest['reconciliation_head'],'predicate_registry_hash':manifest['predicate_registry_hash'],'scope_key_registry_hash':manifest['scope_key_registry_hash'],'compiler_commit':manifest['compiler_commit'],
   'derived_schema_version':DERIVED_SCHEMA_VERSION,'derived_builder_identity':builder_identity,'verification_receipt_hash':verified['receipt_hash'],'verification_receipt_json':canonical(verified['receipt']),'imported_output_contract':canonical(list(bundle.REQUIRED_OUTPUTS)),
  }
  con.executemany('INSERT INTO metadata(key,value) VALUES (?,?)',sorted((k,str(v)) for k,v in metadata.items()))
  for row in rows['entities.jsonl']:
   con.execute('INSERT INTO entities VALUES (?,?,?,?,?,?)',(row['local_entity_id'],row.get('work_id'),row.get('label'),json_value(row.get('identity_links')),json_value(row.get('resolved_scope')),canonical(row)))
  for filename,partition in ASSERTION_PARTITIONS.items():
   for row in rows[filename]:
    con.execute('INSERT INTO assertions VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',(row['assertion_id'],row['projection_status'],partition,row.get('subject_type'),row.get('subject'),row.get('predicate'),json_value(row.get('object')),json_value(row.get('scope')),json_value(row.get('resolved_scope')),row.get('effective_assertion_status'),canonical(row)))
  for row in rows['assertion_history.jsonl']:
   con.execute('INSERT INTO assertion_history VALUES (?,?,?,?)',(row['assertion_id'],row.get('status'),row.get('supersedes'),canonical(row)))
  sources={}; works={}; evidence={}
  for row in rows['provenance.jsonl']:
   con.execute('INSERT INTO provenance VALUES (?,?,?,?)',(row['provenance_id'],row['assertion_id'],row.get('evidence_id'),canonical(row)))
   add_catalog(sources,'source_id',row.get('source_record'),'source'); add_catalog(works,'work_id',row.get('work_record'),'work'); add_catalog(evidence,'evidence_id',row.get('evidence_record'),'evidence')
  ...
  for row in rows['reconciliation_history.jsonl']:
   con.execute('INSERT INTO reconciliation_history VALUES (?,?,?,?,?,?,?,?)',(row['decision_id'],row.get('decision_type'),row.get('subject_type'),row.get('subject_id'),row.get('status'),row.get('supersedes'),json_value(row.get('payload')),canonical(row)))
  for row in rows['relations.jsonl']:
   con.execute('INSERT INTO relations VALUES (?,?,?,?,?,?,?,?)',(row['relation_id'],row.get('relation_kind'),row.get('subject_type'),row.get('subject_id'),row.get('predicate'),row.get('target_type'),row.get('target_id'),canonical(row)))
  expected_active=sum(len(rows[n]) for n in ASSERTION_PARTITIONS); actual=con.execute('SELECT count(*) FROM assertions').fetchone()[0]
  if actual!=expected_active:raise ValueError('SQLite active assertion count mismatch')
