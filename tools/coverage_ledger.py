...
Coverage events never infer later states. A report denominator is RESOLVED only when an
explicit governed denominator snapshot is supplied; the mere presence of Work records is
not evidence that the corpus denominator is complete.
"""
...
def validate_coverage(records,works=None,sources=None,bindings=None):
 ...
 for eid,row in events.items():
  ...
  if row.get('supersedes'):
   ...
   if predecessor is None:errors.append(f'{eid}: supersedes missing coverage event {row["supersedes"]}')
    ...
    if row.get('status') in {'ACCEPTED','REJECTED'} and (not isinstance(row.get('reason'),str) or not row.get('reason').strip()):errors.append(f'{eid}: correction requires non-empty reason')
 for start in events:
 ...
 return errors
def normalize_denominator_snapshot(snapshot,works):
 if snapshot is None:return None
 if not isinstance(snapshot,dict):raise ValueError('denominator snapshot must be an object')
 required=('snapshot_ref','definition','work_ids','exclusions','unknowns');missing=[key for key in required if key not in snapshot]
 if missing:raise ValueError('denominator snapshot missing '+', '.join(missing))
 if not isinstance(snapshot['snapshot_ref'],str) or not snapshot['snapshot_ref'].strip():raise ValueError('denominator snapshot_ref must be non-empty')
 if not isinstance(snapshot['definition'],str) or not snapshot['definition'].strip():raise ValueError('denominator definition must be non-empty')
 if not isinstance(snapshot['work_ids'],list) or any(not isinstance(w,str) or not w for w in snapshot['work_ids']):raise ValueError('denominator work_ids must be string array')
 if len(snapshot['work_ids'])!=len(set(snapshot['work_ids'])):raise ValueError('denominator work_ids contain duplicates')
 if not isinstance(snapshot['exclusions'],list) or not isinstance(snapshot['unknowns'],list):raise ValueError('denominator exclusions and unknowns must be arrays')
 missing_works=sorted(set(snapshot['work_ids'])-set(works))
 if missing_works:raise ValueError(f'denominator snapshot references missing governed Work IDs: {missing_works}')
 return {'snapshot_ref':snapshot['snapshot_ref'],'definition':snapshot['definition'],'work_ids':sorted(snapshot['work_ids']),'exclusions':snapshot['exclusions'],'unknowns':snapshot['unknowns']}
def coverage_report(records,works=None,denominator_snapshot=None):
 works=works if works is not None else index_records(records,'work','work_id');events=index_records(records,'coverage_event','coverage_event_id');active=active_events(events);denom=normalize_denominator_snapshot(denominator_snapshot,works)
 result={'denominator_status':'RESOLVED' if denom is not None else 'DENOMINATOR_UNRESOLVED','denominator_definition':denom['definition'] if denom else None,'denominator_snapshot_ref':denom['snapshot_ref'] if denom else None,'denominator':len(denom['work_ids']) if denom else None,'known_work_record_count':len(works),'exclusions':denom['exclusions'] if denom else [],'unknowns':denom['unknowns'] if denom else ['No governed denominator snapshot supplied'],'history_event_count':len(events),'active_accepted_event_count':len(active),'states':{}}
 denominator_ids=set(denom['work_ids']) if denom else None
 for state in STATES:
  rows=[r for r in active.values() if r.get('coverage_state')==state];all_ids=sorted({r.get('work_id') for r in rows if r.get('work_id') in works});numerator_ids=sorted(set(all_ids)&denominator_ids) if denominator_ids is not None else all_ids;outside=sorted(set(all_ids)-denominator_ids) if denominator_ids is not None else []
  result['states'][state]={'event_count':len(rows),'numerator':len(numerator_ids),'denominator':len(denominator_ids) if denominator_ids is not None else None,'covered_work_ids':numerator_ids,'outside_denominator_work_ids':outside,'coverage_fraction':(len(numerator_ids)/len(denominator_ids) if denominator_ids else None)}
 return result
def main():
 ap=argparse.ArgumentParser();sub=ap.add_subparsers(dest='command',required=True);sub.add_parser('validate');report_parser=sub.add_parser('report');report_parser.add_argument('--denominator-json');args=ap.parse_args();records=scan_typed_records()
 if args.command=='validate':
  errors=validate_coverage(records)
  if errors:print('COVERAGE VALIDATION FAILED');print('\n'.join('- '+e for e in errors));raise SystemExit(1)
  print('COVERAGE VALIDATION PASSED')
 else:
  denominator=json.loads(Path(args.denominator_json).read_text(encoding='utf-8')) if args.denominator_json else None
  print(json.dumps(coverage_report(records,denominator_snapshot=denominator),indent=2,sort_keys=True))
