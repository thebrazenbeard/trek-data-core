...
class ValidationTests(unittest.TestCase):
    ...
    def run_records(self, records):
        with tempfile.TemporaryDirectory() as td:
            research = Path(td) / "research"; research.mkdir()
    ...
    def write_batch_fixture(self, research, batch_hash, worker_id="TNG", record_counts=None, source_work_in_batch=False):
        ...
        batch = research / "tng" / "batches" / "batch-1"; batch.mkdir(parents=True)
        if source_work_in_batch:
            ...
            registry = research / "_registry"; registry.mkdir(parents=True)
        ...
        if record_counts is None: record_counts = default_counts
        manifest = {
    ...
    def test_schema_invalid_source_is_rejected(self):
        rc, output = self.run_records([{"record_type": "source", "source_id": "source-1", "source_kind": "transcript"}]); self.assertEqual(rc, 1); self.assertIn("locator", output)

    def test_dangling_assertion_evidence_reference_is_rejected(self):
        rc, output = self.run_records([{"record_type": "assertion", "assertion_id": "assertion-1", "subject": "local-1", "predicate": "CLAIMS", "object": "x", "evidence": ["evidence-missing"], "status": "ACCEPTED"}]); self.assertEqual(rc, 1); self.assertIn("evidence-missing", output)
    ...
    def test_batch_hash_mismatch_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            rc, output = self.run_research_root(research); self.assertEqual(rc, 1); self.assertIn("batch_hash", output)
    ...
    def test_canonical_batch_hash_is_accepted(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            rc, output = self.run_research_root(research); self.assertEqual(rc, 0, output)
    ...
    def test_worker_id_must_match_research_partition(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            rc, output = self.run_research_root(research); self.assertEqual(rc, 1); self.assertIn("worker_id", output); self.assertIn("TNG", output)

    def test_core_batch_count_keys_are_required(self):
        with tempfile.TemporaryDirectory() as td:
            research = Path(td) / "research"; self.write_batch_fixture(research, MISSING_ASSERTION_COUNT_HASH, record_counts={"local_entities": 1, "evidence": 1})
            rc, output = self.run_research_root(research); self.assertEqual(rc, 1); self.assertIn("record_counts.assertions", output)

    def test_untyped_json_in_governed_data_root_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            research = Path(td) / "research"; artifact = research / "tng" / "coverage_update.json"; artifact.parent.mkdir(parents=True)
            artifact.write_text(json.dumps({"coverage": "SEMANTICALLY_ANALYZED"}) + "\n", encoding="utf-8")
            rc, output = self.run_research_root(research); self.assertEqual(rc, 1); self.assertIn("missing record_type", output); self.assertIn("coverage_update.json", output)

    def test_worker_batch_cannot_own_source_or_work_records(self):
        with tempfile.TemporaryDirectory() as td:
            research = Path(td) / "research"; self.write_batch_fixture(research, ILLEGAL_SOURCE_WORK_BATCH_HASH, source_work_in_batch=True)
            rc, output = self.run_research_root(research)
            self.assertEqual(rc, 1); self.assertIn("Librarian/Consolidator-owned", output); self.assertIn("source", output); self.assertIn("work", output)
